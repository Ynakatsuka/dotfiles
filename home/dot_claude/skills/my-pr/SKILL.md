...
description: >-
  ...
  review comments, reviewing others' or external PRs (use the built-in /review
  instead), or a lightweight review of the current working diff without the PR
  workflow (use the built-in /code-review instead).
...
このスキルは最初のスクリプト実行前に skill directory を自動解決する。
解決した directory は `MY_PR_SKILL_DIR` に保持し、以後の bundled scripts 参照に使う。
推測で `/scripts/...` へ進まない。候補 directory と required scripts を検証し、どの候補も使えない場合だけ停止する。

```bash
resolve_my_pr_skill_dir() {
  local candidates=()
  local dir

  [ -n "${MY_PR_SKILL_DIR:-}" ] && candidates+=("$MY_PR_SKILL_DIR")
  [ -n "${CLAUDE_SKILL_DIR:-}" ] && candidates+=("$CLAUDE_SKILL_DIR")
  [ -n "${HOME:-}" ] && candidates+=("$HOME/.claude/skills/my-pr")

  for dir in "${candidates[@]}"; do
    if [ -f "$dir/SKILL.md" ] && [ -x "$dir/scripts/prepare-review-artifacts.sh" ]; then
      printf '%s\n' "$dir"
      return 0
    fi
  done

  echo "my-pr skill directory could not be resolved. Checked MY_PR_SKILL_DIR, CLAUDE_SKILL_DIR, and \$HOME/.claude/skills/my-pr." >&2
  return 1
}

MY_PR_SKILL_DIR="$(resolve_my_pr_skill_dir)" || {
  unset MY_PR_SKILL_DIR
  exit 1
}
export MY_PR_SKILL_DIR
```

以後、bundled scripts は必ず `${MY_PR_SKILL_DIR:?}/scripts/...` で参照する。
...
- Reviewer A: integrated simplify review (Codex medium effort, chunked for large diffs, capped findings)
...
- Simplify は full diff を優先する。`references/review.md` の保守的な chunking 条件を満たす場合だけ chunking し、各 run/chunk の Required と Recommended をそれぞれ最大 5 件に制限する。
- Codex correctness review は `/my-agent codex` を使う。
- Claude correctness review は host-aware に実行する。Claude Code Agent が使えるセッションでは Agent を使い、それ以外では Claude CLI の `claude --permission-mode plan --tools Read --output-format=stream-json --verbose -p` を使う。
