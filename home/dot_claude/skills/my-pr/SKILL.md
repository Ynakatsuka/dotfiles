...
このスキルは `${CLAUDE_SKILL_DIR}` に依存する。Claude Code のみがこの変数を設定する。最初のスクリプト実行前に host を確認する。

```bash
: "${CLAUDE_SKILL_DIR:?my-pr requires Claude Code (CLAUDE_SKILL_DIR is not set)}"
```

Codex など `${CLAUDE_SKILL_DIR}` が未設定の host では、ここで停止する。パスが `/scripts/...` に誤解決されるまま続行しない。
