...
この経路は共有可能な branch / PR を検証し、コピー可能なプロンプトを作る。公開済みの参照で再開できるなら、引き継ぎ用のブランチを余分に作らない。必要な変更がローカルにしかなければ、検証して commit し、topic branch に push する。`my-handoff` の依頼をこの commit / push の許可として扱い、確認を挟まない。PR の作成・編集は含めない。
...
3. 引き継ぎに必要な変更が未コミットなら、対象ファイルだけを stage し、staged diff と秘密情報の混入を確認してから、関連するローカル検証を行い commit する。無関係な変更を含めない。現在のブランチが保護ブランチなら、topic branch を作ってから commit する。必要な変更を安全に分離できない場合は停止する。
4. 共有が必要な commit が remote にない場合は、push 前に script と同じ remote URL、Git LFS、submodule の条件と remote / PR 照会を確認する。設定済みの push 先を確認し、なければ設定済み remote が一つだけの場合に限ってそれを使い、topic branch に明示的な refspec で push する。ローカル検証を終え、remote の既存 commit を上書きしない。保護ブランチへの push や PR の作成・編集が必要な場合は停止して理由を報告する。
5. リポジトリ内で次を実行する。
...
6. 共有 commit に含まれるか不明な完了事項だけ、対象を絞って照合する。引き継ぎのためにコードやログを広く読み直さず、再開先で調査できる内容は参照箇所を渡す。検証結果は対象の状態と実際に確認できたものだけを書く。
7. 下記の形式で、共有する remote branch または PR と、コピー可能な handoff prompt を返す。内部で実行したコマンド、確認手順、raw output は表示しない。

## 停止条件

script が失敗した場合、引き継ぎ用の commit / push で解消できる状態は手順3・4に従って解消し、再実行する。解消できなければ handoff prompt を作らず、共有できない状態を具体的に報告して停止する。

- 未コミットまたは未追跡の変更: 必要な変更を安全に分離・commit できない
- upstream なし、remote branch なし、SHA 不一致: 安全な topic branch への push でも現在の commit を共有できない
...
引き継ぎに必要な commit / push の許可やブランチ名は聞き直さない。認証・権限の失敗や安全な push 先がない場合は停止し、失敗を報告する。
