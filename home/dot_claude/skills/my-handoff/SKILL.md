...
- `/my-handoff [追加事項]`: [別マシン向け手順](references/remote.md)だけを読む。共有可能な branch / PR とコピー用プロンプトを返す。
...
- `orca` の新規ワークスペースはOrcaにブランチを作らせる。引き継ぎ用の commit に topic branch が必要な場合を除き、現在のブランチは切り替えない。既存の同名セッションを推測で再利用しない。
- `my-handoff` の依頼には、引き継ぎに必要な変更の commit と topic branch への push の許可を含める。必要な場合は確認を挟まず、検証後に実行する。無関係な変更、保護ブランチへの push、PR の作成・編集、引き継ぎ先での後続作業には許可を広げない。
