...
- Local config file: `$HOME/.config/ccv-preview/local.env`
- Service: loaded as `CCV_PREVIEW_SERVICE`
- Local origin: loaded as `CCV_PREVIEW_LOCAL_ORIGIN`
- Public URL: loaded as `CCV_PREVIEW_PUBLIC_URL`
- Listen check pattern: loaded as `CCV_PREVIEW_LISTEN_PATTERN`
- Environment override directory: loaded as `CCV_PREVIEW_ENV_DIR`
- Environment override file: loaded as `CCV_PREVIEW_ENV_FILE`
...
実行前に、暗号化 dotfiles から deploy される local config を source する。
local config が存在しない、または必須値が未設定の場合は停止する。別 URL や別 service へ推測で fallback しない。

```bash
CCV_PREVIEW_LOCAL_CONFIG="$HOME/.config/ccv-preview/local.env"
if [ -f "$CCV_PREVIEW_LOCAL_CONFIG" ]; then
  set -a
  . "$CCV_PREVIEW_LOCAL_CONFIG"
  set +a
fi
: "${CCV_PREVIEW_SERVICE:?Set CCV_PREVIEW_SERVICE in $CCV_PREVIEW_LOCAL_CONFIG}"
: "${CCV_PREVIEW_PUBLIC_URL:?Set CCV_PREVIEW_PUBLIC_URL in $CCV_PREVIEW_LOCAL_CONFIG}"
: "${CCV_PREVIEW_LOCAL_ORIGIN:?Set CCV_PREVIEW_LOCAL_ORIGIN in $CCV_PREVIEW_LOCAL_CONFIG}"
: "${CCV_PREVIEW_LISTEN_PATTERN:?Set CCV_PREVIEW_LISTEN_PATTERN in $CCV_PREVIEW_LOCAL_CONFIG}"
: "${CCV_PREVIEW_ENV_DIR:?Set CCV_PREVIEW_ENV_DIR in $CCV_PREVIEW_LOCAL_CONFIG}"
: "${CCV_PREVIEW_ENV_FILE:?Set CCV_PREVIEW_ENV_FILE in $CCV_PREVIEW_LOCAL_CONFIG}"
```
