...
description: >-
  ...
  Do NOT use for simple bug fixes, one-off small refactors, or tasks that don't need a
  spec-driven approach — refactoring issues created by my-refactor ARE consumed via this
  skill (my-refactor routes each issue to /my-sdd). Do NOT use for multi-PR epic-scale
  work (use my-epic).
...
```
Phase 1 (Plan):  1-0 Investigate → 1-1 Clarify → 1-2 Specify → 1-3 Design → 1-4 Document & Codex Review
                 ↓ ユーザー承認
...
### 1-4 Document & Codex Review

document review と codex review の 2 段で構成する。まず `document-review.md` の **Spec review** を実行し、実装計画に進める粒度か確認する。

- `Status: Approved` → Codex review へ進む
...
`codex` を自動起動して red-team review を行う。詳細手順・プロンプト・反映ルールは `external-review.md`。

**反映ルール（要旨）:**
- 挙動・データモデル不変の指摘 → **自動反映**（用語統一、Open Questions 追記、構造整理など）
- 挙動・API・スキーマ・移行戦略の変更を伴う指摘 → **ユーザー合意の上で反映**
- 不採用 → `docs/specs/{name}/decisions.md` に理由記録
- 判断が微妙なら確認側に倒す
- **Codex review 成功時のみ続行可、失敗時は Blocked**（ユーザー override 必須）
...
保護ブランチ（`main` / `master` / `staging` / `develop` / `production` / `release/*`）から実装しない。検出時は `feat-{feature-name}` で worktree を作成し `docs/specs/{name}/` を移送（`worktree-gate.md`）。
...
📁 出力:
  - docs/specs/{name}/{requirements.md, design.md, decisions.md}
  - /tmp/sdd-reviews/{name}/codex.md

🔎 Codex review: 自動反映 A / 合意反映 B / 不採用 C（docs/specs/{name}/decisions.md 参照）
