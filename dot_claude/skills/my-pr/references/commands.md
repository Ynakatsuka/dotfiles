...
2. Run `eval "$(${CLAUDE_SKILL_DIR}/scripts/prepare-review-artifacts.sh "$BASE_BRANCH")"`.
3. Read `MY_PR_SCOPE_SUMMARY`. Source `MY_PR_ARTIFACT_ENV` if resuming in a new shell.
4. If `MY_PR_SCOPE_GATE` is not `ok`, stop.
...
5. Use the generated repo-local artifacts for review and PR body work. Do not use `/tmp` review patches.
