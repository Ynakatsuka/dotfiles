...
時間は十分かけてよい。`Explore` subagent で並列調査を活用し、「コードで分かることは聞かない」を徹底して Phase 1-1 の質問削減と Phase 1-3 の設計判断材料に使う。調査結果は `design.md` の「既存コードとの統合」「Codebase coherence」「Blast radius」セクションに**ファイルパス・モジュール名・PR 番号付き**で反映する。
...
セクション 10（Codebase Coherence）の根拠を集めるための初期調査:
...
- [ ] タイムアウト、ネットワーク断、部分失敗
- [ ] リトライによる重複（idempotency キー、exactly-once 性）
...
- [ ] 公開 API / モジュール境界の契約
- [ ] 後方互換・破壊的変更の扱い
- [ ] upstream callers / downstream consumers の影響範囲
...
一般論ではなくこのプロジェクト固有の事情に踏み込んで、新機能が既存コードの秩序・規約・既存実装と噛み合っているかを確認する。Phase 1-0 B の調査結果を根拠に判断する。
...
上記チェックの結果として以下を `design.md` に必ず含める:

| セクション | 由来 | 中身 |
|---|---|---|
| Non-functional requirements (NFR) | 2 | 数値目標 |
| Security | 3 | 方針 |
| Observability | 5 | 計測点 |
| Operations / Rollout / Rollback / Migration | 4 | 手順 |
| Failure modes | 1 | 主要シナリオと挙動 |
| Blast radius | 6 | 影響範囲 |
| Codebase coherence | 10 | 既存類似実装の有無と差分の根拠 / 再利用する既存資産（helper / shared module / fixture / pattern） / 共有契約（公開 API / DB schema / 共通型 / shared lib）への破壊的影響と互換戦略 / 逸脱する既存規約とその理由 |
| Risks & Mitigations | 1-10 | 抽出されたリスクと緩和策 |
| Rejected alternatives | 8 | 代替案と不採用理由 |
